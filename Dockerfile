...
# Create monero user
RUN adduser --system --group --disabled-password tube && \
	mkdir -p /wallet /home/tube/.tube && \
	chown -R tube:tube /home/tube/.tube && \
	chown -R tube:tube /wallet
...
# cd /wallet
# bittubecash-wallet-cli
VOLUME /wallet

EXPOSE 18080
EXPOSE 18081

ENTRYPOINT ["bittubecashd", "--p2p-bind-ip=0.0.0.0", "--p2p-bind-port=19949", "--rpc-bind-ip=0.0.0.0", "--rpc-bind-port=19281", "--non-interactive", "--confirm-external-bind"]
